...
{
    public partial class UC_Nidux_CODISA : UserControl
    {
                            ...
                            while (contador1 < lista.Count())
                            {
                                ...
                                if (lista[contador1].id == "")
                                {
                                    ...
                                    string jsonBody = $@"{{
                                        ""add"": [
                                            {{                        
                                                {(lista[contador1].id_marca != null && lista[contador1].id_marca != "0" ? $@"""brand_id"": {lista[contador1].id_marca}," : "")}
                                                ""categorias"": [],
                                                ""product_code"": ""{code}"",
                                                ""product_name"": ""{name}"",
                               ...
                               ===========================================");
                                    ...
                                    var client1 = new RestClient("https://api.nidux.dev/v3/products/");
                                    client1.Timeout = -1;
       ...
       ===========================================");
                ...
                try
                {
                    ...
                    else
                    {
                        foreach (var padre in listaPadres)
                        {
                            ...
                            var atributos = new Clases.Atributos_Padre();
                            ...
                            if ((int)response_atributos2.StatusCode == 200)
                            {
                                ...
                                if (encapsulador_json_variacion.variations.Count != 0)
                                {
                                    ...
                                    if ((int)response_variacion.StatusCode == 200)
                                    {
                                        ...
                                        foreach (var itemAct in variaciones)
                                        {
                                        ...
                                        foreach (var variacion in variaciones)
                                        {
                                            ...
                                            SqlCommand cmd = new SqlCommand();
                                            cmd.Connection = cnn.AbrirConexion();
                                            cmd.CommandText = "" + com + ".ActualizarBanderaArticulo";
                                            cmd.CommandTimeout = 0;
                                            cmd.CommandType = CommandType.StoredProcedure;
                    ...
                    MessageBox.Show("Proceso de sincronización finalizado", "Aviso");
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.Message.ToString(), "Mensaje de Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        public List<Clases.Articulos> Devolver_Padres_Variaciones()
        {

            SqlCommand cmd = new SqlCommand();
            var listaPadres = new List<Clases.Articulos>();

            try
            {
                cmd.Connection = cnn.AbrirConexion();
                cmd.CommandText = $"{com}.CONSULTAR_PADRES_VARIACIONES";
                cmd.CommandTimeout = 0;
                cmd.CommandType = CommandType.StoredProcedure;
            ...
            {
                MessageBox.Show($"Error al consultar padres de variaciones error: {ex.Message.ToString()}");
            }
            finally
            {
                cnn.CerrarConexion();
        ...
        public int[] Devolver_Atributos_Padre(string padre)
        {
            ...
            try
            {
                cmd.Connection = cnn.AbrirConexion();
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandText = $"{com}.CONSULTAR_ATRIBUTOS_POR_PADRE";
            ...
            {
                MessageBox.Show($"Error al consultar atributos del articulo padre: {padre} error: {ex.Message.ToString()} ");
            }
            finally
            {
                cnn.CerrarConexion();
        ...
        {
            var client_atributos = new RestClient($"https://api.nidux.dev/v3/products/{productoId}/attribs");
        ...
        public List<Clases.Variaciones> Devolver_Variaciones_Por_Padre(string padre)
        {

            SqlCommand cmd = new SqlCommand();
            var listaVariaciones = new List<Clases.Variaciones>();

            try
            {
                cmd.Connection = cnn.AbrirConexion();
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandText = $"{com}.CONSULTAR_VARIACIONES_POR_PADRE";
                cmd.Parameters.AddWithValue("@ID_PADRE", padre);
                SqlDataReader rdr = cmd.ExecuteReader();
                while (rdr.Read())
                {
                    Clases.Variaciones list = new Clases.Variaciones();
            ...
            {
                MessageBox.Show($"Error al consultar las variaciones del articulo: {padre} error: {ex.Message.ToString()}");
            }
            finally
            {
        ...
        public int[] Devolver_Atributos_Articulo(string sku)
        {
            ...
            try
            {
                cmd.Connection = cnn.AbrirConexion();
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandText = $"{com}.CONSULTAR_VALORES_ATRIBUTOS_POR_ARTICULO";
            ...
            {
                MessageBox.Show($"Error al obtener los atributos del articulo: {sku} error: {ex.Message.ToString()} ");
            }
            finally
            {
                cnn.CerrarConexion();
        ...
        public void EliminarUnaVariacionQueFuePadreAlInsertarse(string sku)
        {
            ...
            try
            {
                v_Comando.Connection = cnn.AbrirConexion();
                v_Comando.CommandText = com + ".ACTUALIZAR_ARTICULO_DESPUES_DE_INSERTARSE_COMO_VARIACION";
        ...
        private void btnOracleSincro_Click(object sender, EventArgs e)//listo
        {
            ...
            try
            {
                ...
                string query = "SELECT NO_ARTI, DESCRIPCION, PESO, CARACTERISTICAS, PRECIO, PORC_DESC, EXISTENCIA, NIDUX_COD_CATEGORIA, NIDUX_COD_MARCA, DESTACADO, TAGS, SEO_TAG, URL_VIDEO, RECORDDATE, ESTADO FROM naf5m.NIDUX_OBTENER_PRODUCTO_HOGAR";
       ...
       ===========================================");
                            ...
                            {
                                category_name = list_Cat.nombre,
                                category_father = string.IsNullOrEmpty(list_Cat.categoria_padre) ? "0" : list_Cat.categoria_padre,
                                category_description = $"<p>{list_Cat.descripcion}</p>",
                                category_status = 1,
                                category_weigth = 0,
                                mallId = 0,
                                traducciones = new[]
{
                                        new
                                        {
                                            ...
                                            category_description = "<p>Default description</p>"
                                        }
                                    }
                            };


                            string jsonBody = JsonConvert.SerializeObject(categoryData);
                                 */
       ...
       ===========================================");
                            ...
                            // Actualizamos la fecha
                            //LO COMENTO PORQUE SOLO HAY UNA FECHA Y INTERRUMPIRIA LA SINCRONIZACION
                    ...
                    if ((int)response.StatusCode == 200)
                    {
                        ...
                        if ((int)response2.StatusCode == 200)
                        {
                            ...
                            if (lista_respuesta.marcas.Count > 0)
                            {
                                ...
                                foreach (var itemMarcas in lista_respuesta.marcas)
                                {
                                    cmd.Connection = cnn.open();
                                    cmd.CommandText = "NAF5M.PKG_NIDUX_WEB.update_marcas";
                                    cmd.CommandTimeout = 0;
                                    cmd.CommandType = CommandType.StoredProcedure;
                                    cmd.Parameters.Add("PDESCRIPCION", OracleType.VarChar, 30).Value = itemMarcas.Value.brand_name;
                                    cmd.Parameters.Add("PCOD_MARCA_NIUDX", OracleType.Number, 5).Value = itemMarcas.Value.id;
                                    cmd.Parameters.Add("pMENSAJE", OracleType.VarChar, 1000).Direction = ParameterDirection.Output;
                                    cmd.ExecuteNonQuery();
                                    cmd.Parameters.Clear();
                                    cnn.close();
                        ...
                        {
                            MessageBox.Show($"Error en el Api Propio Método insertar marcas a tablas, ERROR: {response2.Content}", "Mensaje de Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if ((int)response.StatusCode == 200)
                    {
                        ...
                        if ((int)response2.StatusCode == 200)
                        {
                            ...
                            foreach(var itemAtrib in atributos.atributos)
                            {
                                var client3 = new RestClient("https://api.nidux.dev/v3/attributes/" + itemAtrib.Value.id + "/values");
                                client3.Timeout = -1;
                                var request3 = new RestRequest(Method.GET);
                                ...
                                if ((int)response3.StatusCode == 200)
                                {
                                    ...
                                    foreach(var itematribValores in atributos_valores.atributo_valores)
                                    {
                                        Clases.Valores valores_atributo = new Clases.Valores();
                                        valores_atributo.id_atributo = itemAtrib.Value.id;
                                        valores_atributo.id_valor_atributo = itematribValores.Value.id;
                                        valores_atributo.nombre_valor_atributo = itematribValores.Value.value_name;
                                    ...
                                    if ((int)response4.StatusCode == 200)
                                    {
                                        mensaje = itemAtrib.Value.attribute_name + "," + mensaje;
                                    }
                                }
                                n++;
                            }
